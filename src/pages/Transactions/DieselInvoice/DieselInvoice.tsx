...
import { getAllDieselInvoices } from "../../../apis/dieselInvoiceApi";

export const DieselInvoice = () => {
    ...
    const filteredInvoices = invoices.filter(
        (inv) =>
            inv.project?.project_no?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            inv.date?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            inv.is_invoiced?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    } = usePagination(filteredInvoices, rowsPerPage);

    const fetchAndSetInvoices = async () => {
        setLoading(true);
        try {
            const data = await getAllDieselInvoices();
            setInvoices(data);
        } catch (err) {
            toast.error("Failed to fetch Diesel Invoices");
        }
        setLoading(false);
    };
    ...
    const exportInvoicesToExcel = (invoices: any[]) => {
        ...
        const exportData = invoices.map((inv) => ({
            ...
            "Project No": inv.project?.project_no || "N/A",
            "Invoice Status": inv.is_invoiced || "N/A",
            "Items Count": inv.formItems?.length || 0,
            "Total Value": inv.formItems?.reduce((sum: number, item: any) => sum + (item.total_value || 0), 0) || 0,
        }));

        const worksheet = XLSX.utils.json_to_sheet(exportData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Diesel Invoices");

        XLSX.writeFile(workbook, "diesel_invoices.xlsx");
    };
    ...
    const handleSortByStatus = () => {
        setInvoices((prev) =>
            [...prev].sort((a, b) =>
                (a.is_invoiced || "").localeCompare(b.is_invoiced || "")
            )
        );
        toast.info("Sorted by Status");
    };

    return (
        <>
            <div className="flex justify-between items-center px-6 mb-2 ">
                <PageBreadcrumb pageTitle="Diesel Invoices" />
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-600 text-gray-800 dark:text-gray-100 text-center">
                                {paginatedInvoices.map((inv, index) => (
                                    <tr
                                        ...
                                        onClick={() =>
                                            navigate(`/diesel-invoice/${inv.id}`, {
                                                state: { invoice: inv },
                                            })
                                        }
                                    >
                                        ...
                                        <td className="px-4 py-3 text-[12px]">
                                            {inv.project?.project_no || "N/A"}
                                        </td>
                                        <td className="px-4 py-3 text-[12px]">
                                            <span className={`px-2 py-1 rounded-full text-xs ${inv.is_invoiced === "invoiced"
                                                ? "bg-green-100 text-green-800"
                                                : "bg-gray-100 text-gray-800"
                                                }`}>
                                                {inv.is_invoiced || "Draft"}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-[12px]">
                                            {inv.formItems?.length || 0}
                                        </td>
                                        <td className="px-4 py-3 text-[12px]">
                                            ₹{inv.formItems?.reduce((sum: number, item: any) => sum + (item.total_value || 0), 0) || 0}
                                        </td>
                                    </tr>
                                ))}
