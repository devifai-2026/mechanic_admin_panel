...
import { AccountGroupPostPayload } from "../../types/accountGroupTypes";
import AccountGroupBulkUpload from "./AccountGroupBulkUpload";
import { FaUpload } from "react-icons/fa";

export default function AccountGroupFormPage() {
  const navigate = useNavigate();
  const { id } = useParams();
  const isEdit = Boolean(id);

  const [formData, setFormData] = useState<AccountGroupPostPayload>({
    account_group_code: "",
    account_group_name: "",
  });
  ...
  useEffect(() => {
    if (isEdit && id) {
      ...
      getAccountGroupById(id)
        ...
        .catch(() => toast.error("Failed to load Account Group"))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/accountgroup/view"), 800);
    } catch (err) {
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6">
            ...
            <div className="flex justify-end gap-4 mt-6">
              <button
                type="button"
                onClick={() => navigate("/accountgroup/view")}
