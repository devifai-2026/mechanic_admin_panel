...
import DownloadTemplateButtonForOrganisations from "../../utils/helperFunctions/organisations.excel";

const OrganisationBulkUpload: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        "/organisations/upload/bulk-upload",
        formData,
        {
          headers: {
      ...
      if (status === 201 && data?.results && Array.isArray(data.results)) {
        ...
        data.results.forEach((item: any, index: number) => {
          message += `${index + 1}. Organisation: ${item.org_code || "N/A"}\n   Status: ${item.status || "unknown"
  ...
  return (
    ...
    >
      ...
      <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-6 text-left">
        ...
        <DownloadTemplateButtonForOrganisations />
      </div>
    </div>
  );
};

export default OrganisationBulkUpload;
